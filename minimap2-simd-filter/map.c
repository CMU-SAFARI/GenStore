...
static void collect_minimizers(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, mm128_v *mv, bool exact_flag, double *hash_time)
{
    double hash_time_dummy = 0;
	...
	for (i = n = 0; i < n_segs; ++i) {
		size_t j;
        if(exact_flag){
		    mm_sketch(km, seqs[i], qlens[i], 1, 76, i, mi->flag&MM_I_HPC, mv, exact_flag, &hash_time_s); //TODO: nika: fix the last argument
            *hash_time = hash_time_s + *hash_time;
            //printf("hash_time inside minimizer: %lf \n", *hash_time);
        }
        else
            mm_sketch(km, seqs[i], qlens[i], mi->w, mi->k, i, mi->flag&MM_I_HPC, mv, exact_flag, &hash_time_dummy); //TODO: nika: fix the last argument
...
static mm128_t *collect_seed_hits_heap(void *km, const mm_mapopt_t *opt, int max_occ, const mm_idx_t *mi, const char *qname, const mm128_v *mv, int qlen, int64_t *n_a, 
...
int mm_map_frag2(const mm_idx_t *mi, const mm_idx_t *mi_exact, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const mm_mapopt_t *opt_exact, const char *qname, int SEED_NUM_PER_READ, int MATCH_NUM_PER_READ, int MAX_NUM_MAPPING_LOCATION_PER_READ, int *MappedReadNo, int *Accepted, int *Rejected, int SSWindow, int SSIteration, int SSEditThreshold, const char *qname2, int printSAM, long *exact_cnt, long *total_cnt, int *mismatch_log, double *hash_time)
{
	//int SEED_NUM_PER_READ=7;
	//int MAX_NUM_MAPPING_LOCATION_PER_READ=50000;

    bool exact_match_detected = false;
    int exact_match_per_read = 0;
	...
	if (qlen_sum == 0 || n_segs <= 0 || n_segs > MM_MAX_SEG) return 1;
                    ...
                    if ((i<n_a) && (((int32_t)a[i].y - (int32_t)a[i-1].y) - ((int32_t)a[i].x - (int32_t)a[i-1].x) ==0)) {
                        ...
                        if ((Seed_Num>=SEED_NUM_PER_READ)) {
                            ...
                            int Edits=0;
                                ...
                                int gapo=2; 
                                int gape=1;
                                int i, g = sc_mch, b = sc_mis < 0? sc_mis : -sc_mis; // g>0 and b<0
                                int8_t mat[25] = { g,b,b,b,0, b,g,b,b,0, b,b,g,b,0, b,b,b,g,0, 0,0,0,0,0 };
                                ...
                                for (i = 0; i < tl; ++i) ts[i] = c[(uint8_t)RefSeq[i]]; // encode to 0/1/2/3
                                for (i = 0; i < ql; ++i) qs[i] = c[(uint8_t)seqs[0][i]];
                         // COMMENTING THE SIMD EXACT MATCH   
                               ...
                               // *total_cnt = *total_cnt + 1;
                                exact_match_sse(0, ql, qs, tl, ts, 5, mat, gapo, gape, -1, -1, 0, 0, &ez, &exact_match, &mismatch_cnt);
...
void mm_map2(const mm_idx_t *mi, const mm_idx_t *mi_exact, int qlen, const char *seq, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const mm_mapopt_t *opt_exact,  const char *qname, int SEED_NUM_PER_READ, int MATCH_NUM_PER_READ, int MAX_NUM_MAPPING_LOCATION_PER_READ, int *MappedReadNo, int *Accepted, int *Rejected, int SSWindow, int SSIteration, int SSEditThreshold, const char * qname2, int printSAM, long *exact_cnt, long *total_cnt, int *mismatch_log, double *hash_time)
{
	//int32_t *rs;
	mm_map_frag2(mi, mi_exact, 1, &qlen, &seq, n_regs, b, opt, opt_exact, qname, SEED_NUM_PER_READ, MATCH_NUM_PER_READ, MAX_NUM_MAPPING_LOCATION_PER_READ, MappedReadNo, Accepted, Rejected,  SSWindow, SSIteration, SSEditThreshold, qname2, printSAM, exact_cnt, total_cnt, mismatch_log, hash_time);
	//printf("%d \n", mismatch_log[9]);
...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	hash  = __ac_Wang_hash(hash);
    double hash_time = 0;
	collect_minimizers(b->km, opt, mi, n_segs, qlens, seqs, &mv, false, &hash_time);
	if (opt->flag & MM_F_HEAP_SORT) a = collect_seed_hits_heap(b->km, opt, opt->mid_occ, mi, qname, &mv, qlen_sum, &n_a, &rep_len, &n_mini_pos, &mini_pos);
	else a = collect_seed_hits(b->km, opt, opt->mid_occ, mi, qname, &mv, qlen_sum, &n_a, &rep_len, &n_mini_pos, &mini_pos);
...
static void *worker_pipeline(void *shared, int step, bool exact_flag, void *in)
{
	int i, j, k;
...
int mm_map_file_frag(const mm_idx_t *idx, int n_segs, const char **fn, const mm_mapopt_t *opt, int n_threads)
{
	...
	kt_pipeline(pl_threads, worker_pipeline, &pl, 3, false);
...
int mm_split_merge(int n_segs, const char **fn, const mm_mapopt_t *opt, int n_split_idx)
{
	...
	kt_pipeline(2, worker_pipeline, &pl, 3, false);
