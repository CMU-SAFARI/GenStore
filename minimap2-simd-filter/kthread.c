...
static void *ktp_worker(void *data)
{
	...
	while (w->step < p->n_steps) {
		...
		// working on w->step
		w->data = p->func(p->shared, w->step, w->exact_flag, w->step? w->data : 0); // for the first step, input is NULL
...
void kt_pipeline(int n_threads, void *(*func)(void*, int, bool, void*), void *shared_data, int n_steps, bool exact_flag)
{
	...
	for (i = 0; i < n_threads; ++i) {
		...
		w->step = 0; w->pl = &aux; w->data = 0; w->exact_flag = exact_flag;
		w->index = aux.index++;
	}
