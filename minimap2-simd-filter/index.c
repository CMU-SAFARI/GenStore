...
{
    printf("6: started worker_pipeline\n");
    //TODO: see what besides mm_sketch you need to change in this function.
	int i;
    ...
    if (step == 0) { // step 0: read sequences
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (t->l_seq > 0){
                printf("7: going to sketch\n");
                double hash_time = 0;
				mm_sketch(0, t->seq, t->l_seq, p->mi->w, p->mi->k, t->rid, p->mi->flag&MM_I_HPC, &s->a, exact_flag, &hash_time);
            }
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size, bool exact_flag)
{
	pipeline_t pl;
    ...
    printf("5: before kt_pipeline\n");
	kt_pipeline(n_threads < 3? n_threads : 3, worker_pipeline, &pl, 3, exact_flag);
	if (mm_verbose >= 3)
...
mm_idx_t *mm_idx_build(const char *fn, int w, int k, int flag, int n_threads) // a simpler interface; deprecated
{
	...
	fp = mm_bseq_open(fn);
	if (fp == 0) return 0;
    bool exact_match = false; //nika: this function is deprecated, looks like.
	mi = mm_idx_gen(fp, w, k, 14, flag, 1<<18, n_threads, UINT64_MAX, exact_match);
	mm_bseq_close(fp);
	return mi;
}
...
mm_idx_t *mm_idx_str(int w, int k, int is_hpc, int bucket_bits, int n, const char **seq, const char **name)
{
	...
	for (i = 0, sum_len = 0; i < n; ++i) {
		...
		if (p->len > 0) {
			a.n = 0;
            double hash_time = 0;
			mm_sketch(0, s, p->len, w, k, i, is_hpc, &a, false, &hash_time);
...
mm_idx_t *mm_idx_reader_read(mm_idx_reader_t *r, int n_threads, bool exact_flag)
{
    printf("3: inside mm_idx_reader_read\n");
	mm_idx_t *mi;
	if (r->is_idx) {
        ...
        printf("4: going to generate the idx\n");
		mi = mm_idx_gen(r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size, n_threads, r->opt.batch_size, exact_flag);
    }
	if (mi) {
